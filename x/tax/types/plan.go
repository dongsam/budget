...
//nolint:interfacer
func NewBasePlan(id uint64, typ PlanType, taxPoolAddr, terminationAddr string, coinWeights sdk.DecCoins, startTime, endTime time.Time) *BasePlan {
	basePlan := &BasePlan{
		Id:                 id,
		Type:               typ,
		TaxPoolAddress: taxPoolAddr,
		RewardPoolAddress:  GenerateRewardPoolAcc(PlanName(id, typ, taxPoolAddr)).String(),
...
func (plan BasePlan) GetTaxPoolAddress() sdk.AccAddress {
	addr, _ := sdk.AccAddressFromBech32(plan.TaxPoolAddress)
	return addr
}

func (plan *BasePlan) SetTaxPoolAddress(addr sdk.AccAddress) error {
	plan.TaxPoolAddress = addr.String()
	return nil
}
...
func (plan BasePlan) Validate() error {
	...
	if _, err := sdk.AccAddressFromBech32(plan.TaxPoolAddress); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidAddress, "invalid tax pool address %q: %v", plan.TaxPoolAddress, err)
	}
...
func (plan BasePlan) Name() string {
	return PlanName(plan.Id, plan.Type, plan.TaxPoolAddress)
}

// PlanName returns unique name of the plan consists of given Id, Type and TaxPoolAddress.
func PlanName(id uint64, typ PlanType, taxPoolAddr string) string {
	poolNameObjects := make([]string, 3)
	poolNameObjects[0] = strconv.FormatUint(id, 10)
	poolNameObjects[1] = strconv.FormatInt(int64(typ), 10)
	poolNameObjects[2] = taxPoolAddr
