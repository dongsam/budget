...
func (r PlanRecord) Validate() error {
	...
	if err := r.TaxPoolCoins.Validate(); err != nil {
		return err
	}
...
func (s Staking) Validate() error {
	if _, err := sdk.AccAddressFromBech32(s.Farmer); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidAddress, "invalid tax pool address %q: %v", s.Farmer, err)
	}
...
func (r Reward) Validate() error {
	if _, err := sdk.AccAddressFromBech32(r.Farmer); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidAddress, "invalid tax pool address %q: %v", r.Farmer, err)
	}
